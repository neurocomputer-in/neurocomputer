...
@dataclass
class Config:
    """Configuration for the Neuro CLI client"""
    host: str = "localhost"
    port: int = 7000
    cid: str = ""
    dev_mode: bool = False
...
class NeoClient:
    ...
    def __init__(self, config: Config):
        ...
        self.ws = None
        self.flow_data = None
        self.node_neuro: dict[str, str] = {}   # ← track neuro for each node
        ...
        console.print(f"[bold blue]neuroMetachine Client v1.0[/]")
        console.print(f"[bold]Conversation ID:[/] [green]{self.cid}[/]")
        if config.dev_mode:
            console.print(f"[bold yellow]🛠️  neuroMetachine Dev Mode: ON[/]")
    ...
    async def _send_message(self, text: str):
        """Send a message to the Neuro server"""
        try:
    ...
    def _visualize_dag(self, flow_data: Dict[str, Any]):
        ...
        for node_id, node_data in nodes.items():
            neuro_name = node_data.get("neuro", "unknown")
            params = node_data.get("params", {})
            param_str = "\n".join(f"{k}: {v}" for k, v in params.items())
            label = f"{node_id}\n{neuro_name}\n{param_str}"
            G.add_node(node_id, label=label, neuro=neuro_name)
        ...
        # Node colors based on neuros
        node_colors = []
        for node in G.nodes():
            neuro = G.nodes[node].get("neuro", "")
            if "reply" in neuro:
        ...
        plt.title("Neuro Task Flow (DAG)", size=16)
        plt.axis("off")
        plt.tight_layout()
        ...
        while not self.exit_flag.is_set():
            try:
                ...
                if topic == "user":
                    ...
                    # remember which neuro this node is running
                    self.node_neuro[node_id] = neuro
                    self._display_message(
                        "system",
                        f"▶ Node {node_id}   (neuro: {neuro})",
                    ...
                    node_id = data.get("id")
                    out     = data.get("out", {})
                    # lookup the neuro name we saved earlier
                    neuro   = self.node_neuro.get(node_id, "‽")

                    # trace the completion of the node with its neuro
                    self._display_message(
                        "system",
                        f"✓ Node {node_id}   (neuro: {neuro})",
                        message_type="technical"
    ...
    def _display_help(self):
        ...
        table = Table(title="[bold]Neuro Client Commands[/]", box=box.ROUNDED, border_style="dim cyan")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Neuro CLI Client")
    parser.add_argument(
        "--host", 
        default="localhost",
        help="Host address for the Neuro server"
    )
    parser.add_argument(
        "--port", 
        type=int, 
        default=7000,
        help="Port for the Neuro server"
    )
    parser.add_argument(
        "--cid", 
        default="",
