...
async def run(state, **kwargs):
    ...
    # Helper to replace placeholders in prompt if needed
    # (NeuroFactory might not replace them automatically, usually prompt.txt is static)
    # But usually we pass them as user message or system prompt.
    # BaseBrain.generate_json takes user_msg.
    # Let's construct the user message with the variables.
...
"""
    ...
    # We strip the placeholders from system_prompt if they exist there
    # or just use system_prompt and append our dynamic content.
    # The prompt.txt I wrote has placeholders {{skills}}, {{history}}, {{text}}.
    # Example: system_prompt = prompt_txt.replace("{{skills}}", skills)...
