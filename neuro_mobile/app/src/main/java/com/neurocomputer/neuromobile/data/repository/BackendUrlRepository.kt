...
class BackendUrlRepository @Inject constructor(
    ...
    // Tablet-mode feature flag: when true, mobile orientation changes rotate
    // the PC display via xrandr. When false, the PC stays landscape and the
    // portrait phone view will show a small horizontal strip. Default OFF so
    // users opt in to the desktop-rotation behaviour.
    private val _rotateDesktop = MutableStateFlow(false)
    val rotateDesktop: StateFlow<Boolean> = _rotateDesktop.asStateFlow()
    ...
    private suspend fun loadRotateDesktop() {
        val saved = context.dataStore.data.map { prefs ->
            prefs[ROTATE_DESKTOP_KEY]
        }.first()
        if (saved != null) _rotateDesktop.value = saved
    }

    suspend fun setRotateDesktop(value: Boolean) {
        _rotateDesktop.value = value
        context.dataStore.edit { prefs ->
            prefs[ROTATE_DESKTOP_KEY] = value
        }
    }
    ...
    companion object {
        ...
        private val ROTATE_DESKTOP_KEY = booleanPreferencesKey("rotate_desktop_display")
    }
}
