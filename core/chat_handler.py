...
class ChatRoom:
    ...
    def __init__(
        ...
        self._agent_room_connected = asyncio.Event()
    ...
    async def connect_agent(self):
        ...
        # Disconnect old agent room if exists
        if self.agent_room:
            try:
        ...
        # Unique identity per connection to avoid DuplicateIdentity
        suffix = uuid.uuid4().hex[:6]
        agent_token = self._generate_token(
            f"{self.agent_identity}_{suffix}",
            can_publish=True,
            can_subscribe=True,
            is_agent=True
        )
    ...
    def _agent_connected(self) -> bool:
        """Check if the agent room is actually connected."""
        if not self.agent_room:
            return False
        try:
            return self.agent_room.connection_state == rtc.ConnectionState.CONN_CONNECTED
        except Exception:
            return False

    async def start_agent(self):
        """Start or reconnect the agent for this chat room."""
        async with self._agent_lock:
            if self._agent_started and self._agent_connected():
                return
    ...
    async def handle_user_message(self, msg: ChatMessage):
        ...
        async with self._message_lock:
            ...
            try:
                ...
                err_msg = ChatMessage(msg_type="text", sender="agent",
                                      content="⚠️ Agent startup timed out. Please try again.")
                await self.send_to_all(err_msg, topic="agent_response")
                return

            if self._brain and msg.type in ("text", "voice"):
                # Read fresh agent_id from conversation file (user may have switched)
                fresh_agent_id = self.agent_id
                try:
                    import json as _json
                    from pathlib import Path
                    conv_file = Path(__file__).parent.parent / "conversations" / f"{self.conversation_id}.json"
                    if conv_file.exists():
                        with open(conv_file) as _f:
                            _data = _json.load(_f)
                            if isinstance(_data, dict) and _data.get("agent_id"):
                                fresh_agent_id = _data["agent_id"]
                                if fresh_agent_id != self.agent_id:
                                    logger.info(f"[ChatRoom] agent_id updated: {self.agent_id} → {fresh_agent_id}")
                                    self.agent_id = fresh_agent_id
                except Exception as e:
                    logger.warning(f"[ChatRoom] Could not read fresh agent_id: {e}")

                logger.info(f"[ChatRoom] Calling brain.handle for cid={self.conversation_id}, agent={fresh_agent_id}")
                response_text = await self._brain.handle(
                    cid=self.conversation_id,
                ...
                if response_text:
                    if response_text.startswith("🚀"):
                        logger.info(f"[ChatRoom] Task launched, response will arrive via executor")
                    else:
                        # brain._pub already sent via DataChannel + saved to DB
                        logger.info(f"[ChatRoom] Reply delivered via brain._pub: {response_text[:50]}")
                else:
                    logger.warning(f"[ChatRoom] brain.handle returned empty/None")
                    err_msg = ChatMessage(msg_type="text", sender="agent",
                                          content="⚠️ No response generated. Please try again.")
                    await self.send_to_all(err_msg, topic="agent_response")
    ...
    async def _ensure_agent_connected(self):
        """Ensure agent room is connected, reconnecting if needed. Uses lock to prevent concurrent reconnections."""
        ...
        async with self._reconnect_lock:
            ...
            if self._agent_connected():
                return True
            logger.warning(f"[Broadcast] Agent room not connected, reconnecting...")
            try:
                self._agent_room_connected.clear()
                self._agent_ready.clear()
                await self.connect_agent()
                return True
            except Exception as e:
                logger.error(f"[Broadcast] Failed to reconnect agent: {e}")
                return False

    async def broadcast_to_user(self, msg: ChatMessage, topic: str = "chat_message"):
        """Broadcast message to the room. Reconnects agent if connection is stale."""
        if not await self._ensure_agent_connected():
            return
...
class ChatManager:
    ...
    async def get_or_create_room(
        ...
        async with self._lock:
            if conversation_id in self._rooms:
                room = self._rooms[conversation_id]
                # Update agent_id if it changed (user switched agent)
                if agent_id and agent_id != room.agent_id:
                    logger.info(f"Updating room {conversation_id} agent_id: {room.agent_id} → {agent_id}")
    ...
    async def generate_token(
        ...
        # Start agent and wait for it to join LiveKit room before returning token.
        # This ensures the agent is in the room to receive messages from mobile.
        if not room._agent_started:
            async def _safe_start():
                try:
                    ...
                    room._agent_room_connected.set()  # unblock even on failure
            asyncio.create_task(_safe_start())
        try:
            await asyncio.wait_for(room._agent_room_connected.wait(), timeout=15.0)
        except asyncio.TimeoutError:
            logger.error(f"[ChatManager] Agent room connect timeout for {conversation_id}")
        
        # Use unique identity per connection to avoid LiveKit 403 on duplicate join
        suffix = uuid.uuid4().hex[:6]
        identity = f"agent_{participant_name}" if is_agent else f"user_{participant_name}_{suffix}"
