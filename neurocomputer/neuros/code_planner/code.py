...
# ─────────────────────────────── ALIAS MAP ───────────────────────────────
alias = {
    "write_file":      "code_file_write",       # safer neuro
...
def _wrap_flow(raw_flow):
    ...
    if isinstance(raw_flow, list):
        ...
        for idx, step in enumerate(raw_flow):
            node_id = f"n{idx}"
            # If the step is a dict with an 'action' key, use that as the neuro name
            if isinstance(step, dict) and 'action' in step:
                # Use global alias map to translate friendly aliases to real neuro names
                neuro = alias.get(step['action'], step['action'])
                params = {k: v for k, v in step.items() if k != 'action'}
            else:
                # Otherwise treat the step itself (if string) as the neuro name
                neuro = step if isinstance(step, str) else None
                params = {}
...
def _ensure_repl_reply(flow):
    ...
    if tail_spec.get("neuro") != "code_repl":
        return flow
...
async def run(state, *, goal: str = None, catalogue: list = None, intent: str = None):
    ...
    # ── Intercept capability queries → list neuros ────────────────────────────
    if goal:
        gl = goal.lower().strip()
        capability_queries = [
            "what can you do", "what are your neuros",
            "list neuros", "available neuros", "what and all you can do"
        ]
    ...
    query = {
        ...
        "time": now,
        "system": system,
        "payload": json.dumps({"goal": goal, "neuros": catalogue}, ensure_ascii=False)
    }
    ...
    # ───────────── 2.  ALIAS TRANSLATION FOR SAFER neuro CALLS ─────────
    if isinstance(plan.get("flow"), dict):
        root = plan["flow"]
        neuro = root.get("neuro")
        if neuro in alias:
            new_neuro = alias[neuro]
            root["neuro"] = new_neuro
            params = root.setdefault("params", {})
            if new_neuro == "code_file_write":
                # Normalize param keys
                params["filepath"] = params.pop("path", params.pop("filename", ""))
            elif new_neuro == "code_project_manager":
                # funnel any alias into one canonical key
        ...
        # ── NEW: direct calls to code_project_manager ────────────────────────────
        if neuro == "code_project_manager":
            p = root.setdefault("params", {})
            if "name" in p and "project_name" not in p:
                p["project_name"] = p.pop("name")
            if "path" in p and "project_name" not in p:
                p["project_name"] = p.pop("path")
