...
async def run(state, *, neuro: str | None = None,
                     name:  str | None = None,
                     text:  str | None = None):
    """
    Create – or iteratively refine – a neuro draft via the LLM.

    • First call (no drafts) 👉 generate neuro.json / prompt.txt / code.py.
    ...
    # ───────────────────────────────────────────────────────────────────────
    # 0.   Which neuro are we talking about?
    # ───────────────────────────────────────────────────────────────────────
    chosen = (neuro or name or ctx.get("neuro") or "").strip()
    ...
    if not drafts:
        ...
        chosen = files.get("neuro_name") or chosen or "new_neuro"
        ctx["neuro"] = chosen

        # Write each returned file into the draft buffer
        root = pathlib.Path("neuros") / chosen
        for rel_path, body in files.items():
            if rel_path == "neuro_name":
                continue
