...
class MouseController:
    ...
    def __init__(self, screen_width: int = 1920, screen_height: int = 1080,
        ...
        # ── Direct move serializer — prevents out-of-order xdotool calls ──
        self._direct_move_target: Optional[tuple] = None
        self._direct_move_task: Optional[asyncio.Task] = None
    ...
    async def handle_event(self, event: dict):
        ...
        try:
            ...
            elif event_type == "direct_move":
                await self._handle_direct_move(event)
    ...
    async def _handle_direct_move(self, event: dict):
        """Handle absolute mouse movement (normalized 0-1 coordinates).
        Serialized: only the latest target is executed, older ones are skipped."""
        ...
        if self.dry_run:
            logger.info(f"[Mouse][DRY] mousemove({abs_x}, {abs_y})")
            return

        # Store latest target — always process the most recent position
        self._direct_move_target = (abs_x, abs_y)
        if self._direct_move_task is None or self._direct_move_task.done():
            self._direct_move_task = asyncio.ensure_future(self._process_direct_moves())

    async def _process_direct_moves(self):
        """Process direct moves sequentially, always jumping to the latest target."""
        while self._direct_move_target is not None:
            target = self._direct_move_target
            self._direct_move_target = None
            await asyncio.to_thread(_xdotool, "mousemove", str(target[0]), str(target[1]))
            # Brief pause to batch rapid updates
            await asyncio.sleep(0.008)
            if self._direct_move_target is None:
                break
    ...
    # ─── Tablet-mode absolute touch handlers ─────────────────────────────
